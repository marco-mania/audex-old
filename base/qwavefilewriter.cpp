...
#include "qwavefilewriter.h"

QWaveFileWriter::QWaveFileWriter() {
  data_written = 0;
...
bool QWaveFileWriter::open(const QString& filename) {
...
bool QWaveFileWriter::isOpen() {
  return output_file.isOpen();
}

QString QWaveFileWriter::filename() const {
  return _filename;
}

void QWaveFileWriter::setEndianess(const Endianess e) {
  _endianess = e;
}

const QWaveFileWriter::Endianess QWaveFileWriter::endianess() {
  return _endianess;
}

void QWaveFileWriter::close() {
...
void QWaveFileWriter::write(const QByteArray& data) {
  ...
  if (isOpen()) {
    if (_endianess == LittleEndian) {
      ...
      if (data.size() % 2 > 0) {
        qDebug() << "(QWaveFileWriter) data length is not a multible of 2! Cannot write data.";
        return;
      }
...
void QWaveFileWriter::write_empty_header() {
...
void QWaveFileWriter::update_header() {
  if (isOpen()) {
    output_file.flush();
